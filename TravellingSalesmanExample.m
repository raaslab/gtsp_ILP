...
%         that subtour and all subtours that use those stops.
        ...
        for jj = 1:length(variations)
            whichVar = (sum(idxs==subTourIdx(variations(jj,1)),2)) & ...
                       (sum(idxs==subTourIdx(variations(jj,2)),2));     % test using idx_subu == [117 143; 117 149;143 149] --- subu = [149 117 143]; ---subu_variation = nchoosek(1:3,2);
                                                                        % sum
                                                                        % is actually column vector giving row sum; it's a clever use as takes care of both columns; whichvar is column logical vector 
                                                                        % where idxs matches current choosen edge in variations% 
