...
% relationship..
Aeq = zeros(length(V_Cluster),(length(V_comp_upper)^2 + length(V_comp_upper)));
...
for i = 1:length(Cluster_to_node)
    ...
    Aeq(i,:) = horzcat(adj_V_eq_clus(:)', node_sel); % just attaching node variables with edge variables... node_variables are zero in these but included for uniformity required to push to intlinprog
    
end

Beq = 2*ones(length(Cluster_to_node),1); %**** for the case when  we allow atleast case then this will also become inequality >=2 (check if this is true by considering for 3 4 5 how it behaves)
%%
...
Aeq = [Aeq;Aeq_continuity]; 
Beq = [Beq;Beq_continuity];
...
A  = zeros(counter_max,(length(V_comp_upper)^2 + length(V_comp_upper)));

B  = zeros(counter_max, 1);
...
for j = 2:(length(V_comp_upper)-1)
    ...
    for k = 1:length(nodes_comb)
        ...
        for l = 1:length(node_notin_subset)
            ...
            A(counter,:) = horzcat(adj_V_comp_A(:)', node_notsol);
            B(counter,:) = j;        
            counter = counter + 1;
        end
...
A  = A(1:(counter-1),:); % removing empty ones
B  = B(1:(counter-1),:);
